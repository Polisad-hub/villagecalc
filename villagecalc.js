...
async function initAttackPlanner(groupId) {
    ...
    let destinationVillage = (function() {
    // 1. Zkusíme najít čisté souřadnice v tabulce (pro starý vzhled)
    let coords = "";
    jQuery('#content_value td').each(function() {
    ...
    // 2. Pokud neuspějeme, vezmeme je z nadpisu (pro nový vzhled)
    const headerText = jQuery('#content_value h2').text();
    const match = headerText.match(/\d{1,3}\|\d{1,3}/);
    return match ? match[0] : ""; // [0] zajistí, že dostaneme čistý text
})();
